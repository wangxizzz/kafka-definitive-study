...
public class ConsumerTest {
    /*
     * 发生rebalance时,consumer不可以进行消费,这个时间窗口应该很小,
     * 此外当一个partition被别的消费者消费时,消费者失去了当前的状态,如果它缓存
     * 了一些数据,还需要刷新缓存这会影响应用程序的性能
     *
     * kafka集群中有个broker被设计为group coordinator可以有多个来对应不同的消费者组,
     * 它通过心跳来管理消费者组中消费者之间的关系和他们消费
     * 的partition,只要消费者定期发送心跳它就会认为consumer是活的且能够从他的partition中处理消息,
     * 当消费者发送poll请求数据和commit records
     * it has consumed时发送心跳
     *
     * 如果一个消费者长时间没有发送heartbeats,session会timeout,group coordinator就认为挂了,此时触发rebalance
     * 如果一个消费者crashed并且停止处理消息,group coordinator会等待几秒来决定consumer是否挂了是否要触发rebalance,
     * 这几秒内该partition
     * 的消息不会被处理
     ...
     * 当一个消费者想加入到一个组里面,它发送JoinGroup请求给group coordinator,第一个加入组的消费者成为group leader,
     * 这个leader从group
     * coordinator获取组中所有消费者(包括最近发送过心跳的consumer)的列表并且负责给其他consumer分配partition,
     * 它用PartitionAssignor
     * 的实现类来决定哪个partition归哪个consumer kafka有两种分配partition的策略,当分配结果出来后,
     * group leader把结果列表给GroupCoordinator
     * GroupCoordinator把结果发给其他consumer,其他consumer只能看到自己被分配的partition,group leader
     * 是唯一一个知道所有分配结果(同一组中
...
//        kafkaConsumer.subscribe(Pattern.compile("test.*"));
        try{
            while (true){
                ...
                ConsumerRecords<String, String> consumerRecords = kafkaConsumer.poll(10000);//如果没有数据到consumer buffer 阻塞多久
                //应该在别的线程处理消息
                System.out.println("====================================================");
