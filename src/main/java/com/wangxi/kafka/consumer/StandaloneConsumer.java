...
/**
 * Created by wangxi on 2018/4/27 14:41
 */

public class StandaloneConsumer {
    /*
        之前的消费者都是基于消费者组的,这个例子是单独的消费者可以订阅一个topic或者一个topic的几个partitions,
        如果这样不要用subscribe()
    ...
    public void standalone() {
        ...
        if (partitionInfos != null) {
            ...
            ConsumerTest.kafkaConsumer.assign(partitionSet);  // 消费者自己为自己分区
            while (true) {
                //略
            }
        }
...
//如果有新的partition加进来,这个consumer是不知道的,要定期调用consumer.partitionsFor()来检查是否有partition加进来了