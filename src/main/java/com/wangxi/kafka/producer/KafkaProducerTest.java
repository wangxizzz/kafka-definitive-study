 ...
 * <p>
 * producer有一个buffer space pool来保存还没有被发送到broker的消息,
 * 后台I/O线程负责把这些消息转化成requests请求并发送到集群
 ...
 * 5. broker接收到消息,如果成功保存到磁盘返回RecordMetadata(topic, partition, offset),
 * 失败发送错误信息,当producer收到错误信息后会
 ...
 * <p>
 * KafkaProducer有两种类型的错误. Retriable可以通过重新发送message解决的错误,
 * 比如connection error可以等待重新连接在发送msg,no leader
 * error可以等待leader 被选举出来解决,KafkaProducer可以通过配置来自动重试来应对这些错误,
 * 仅当重试次数达到了抛出Retriable异常,有些错误是不
 * 可以通过retry解决的比如message size too large这种情况不会retry并且直接返回异常
 * <p>
 * kafka的消息顺序保证(partition是有序的) 如果要保证顺序建议设置in.flight.request.per.session=1,
 * 如果发生retry,其他msg不能被发送
 */
